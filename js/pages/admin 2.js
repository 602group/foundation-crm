...
function renderAdmin(params = {}) {
  ...
  function renderPermissionsTab(body) {
    ...
    document.getElementById('btn-mass-export')?.addEventListener('click', async () => {
      const TABLES = [
        'users','auctions','opportunities','donations','courses',
        'tasks','financials','messages','events',
      ];
      Toast.info('Starting mass export — files will download one by one...');
      for (let i = 0; i < TABLES.length; i++) {
        const t = TABLES[i];
        if (Store._getTable(t).length === 0) continue; // skip empty tables
        await new Promise(res => setTimeout(res, 350 * i));
        try { Importer.smartExport(t); } catch(e) { console.warn('Export failed for', t, e); }
      }
      Store.logAudit('mass_export', 'system', 'all tables');
      setTimeout(() => Toast.success('Mass export complete!'), 350 * TABLES.length + 200);
    });
