...
function renderImports(params = {}) {
  ...
  function renderExportTab(body) {
    ...
    body.innerHTML = `
      <div class="feed-card">
        <div class="feed-card-header">Export Data</div>
        <div style="font-size:var(--text-xs);color:var(--text-muted);margin-bottom:var(--space-4)">
          Exports include resolved linked record names (donor names, course names, etc.) — ready for use in Excel.
        </div>
        <div style="display:flex;flex-direction:column;gap:var(--space-2)">
          ${exports.map(item => {
            const count = Store._getTable(item.table).length;
            return `
              <div style="display:flex;align-items:center;gap:var(--space-4);padding:var(--space-3) var(--space-4);background:var(--bg-elevated);border:var(--border-subtle);border-radius:var(--radius-md)">
                <span style="font-size:1.3em">${item.icon}</span>
                <div style="flex:1">
                  <div style="font-size:var(--text-sm);font-weight:var(--fw-medium);color:var(--text-primary)">${item.label}</div>
                  <div style="font-size:var(--text-xs);color:var(--text-muted)">${count} record${count!==1?'s':''}</div>
                </div>
                <button class="btn btn-secondary btn-sm" onclick="Importer.smartExport('${item.table}'); Toast.success('${item.label} exported')">
                  📥 Export CSV
                </button>
              </div>`;
          }).join('')}
